...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	public ShardedJedisSentinelPool(List<String> masters,
		...
		List<HostAndPort> masterList = initSentinels(masters, sentinels);
		initPool(masterList);
	}

	private List<HostAndPort> initSentinels(List<String> masters,
	...
	private void initPool(List<HostAndPort> masters) {
		if (!equals(masters, currentHostMasters)) {
			...
			ShardedJedisFactory factory = new ShardedJedisFactory(
						makeShardInfoList(currentHostMasters),
						Hashing.MURMUR_HASH, keyTagPattern);
